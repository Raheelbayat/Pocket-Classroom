...
// Quiz question row
function createQuestionRow(q = { question: "", choices: ["", "", "", ""], answer: 0, correctIndex: undefined, explanation: "" }) {
  const row = document.createElement("div");
  ...
  `;
  row.querySelector(".correct-select").value = (typeof q.correctIndex !== "undefined") ? q.correctIndex : (typeof q.answer !== "undefined" ? q.answer : 0);
  row.querySelector("button")?.addEventListener("click", () => { row.remove(); autosaveDraft(); });
...
function autosaveDraft() {
  const draft = {
    ...
    quiz: Array.from(document.getElementById("quizContainer")?.children || []).map(c => {
      ...
      return { question: inputs[0]?.value || "", choices:[inputs[1]?.value||"",inputs[2]?.value||"",inputs[3]?.value||"",inputs[4]?.value||""], correctIndex: parseInt(select?.value||0,10), explanation: inputs[5]?.value || "" };
    })
  };
  ...
  init() {
    document.querySelectorAll("a.nav-link").forEach(link => {
      link.addEventListener("click", e => {
        e.preventDefault();
        Router.nav(normalizeRoute(link.getAttribute("href")));
      });
    });

    window.addEventListener("hashchange", () => Router.nav(window.location.hash || "#", false));

    Router.nav(window.location.hash || "#", false);
  },

  nav(route, addToHistory = true) {
    const routeHash = normalizeRoute(route);
    const entry = document.querySelector("#entry");
    if (!entry) return console.error("#entry element missing!");

    if (addToHistory && window.location.hash !== routeHash) window.location.hash = routeHash;

    entry.innerHTML = Router.routes[routeHash]?.() || "<h1>Page not found</h1>";

    // Only init Learn capsules here
    if (routeHash === "#learn") initCapsules();

    // JSON Import buttons
    [document.getElementById("importJsonBtn"), document.getElementById("importJsonBtnAuthor")].forEach(btn => {
      if (!btn) return;
      btn.addEventListener("click", () => {
        ...
        fileInput.addEventListener("change", async e => {
          ...
          try {
            ...
            const wrapped = (parsed.schema && parsed.capsule) ? parsed : { schema:"pocket-classroom/v1", capsule: parsed };
            Storage.importCapsule(wrapped);
            alert(`Imported capsule "${wrapped.capsule?.meta?.title || wrapped.capsule?.title || 'Untitled'}" successfully!`);
            Router.nav("#", false);
    ...
    if (routeHash === "#") {
      ...
      if (!capsules.length) {
        container.innerHTML = `<div class="col-12 p-4 bg-light rounded-3 shadow-sm text-center">
          <h3>No capsules yet</h3><p>Create a capsule or import JSON to get started.</p>
        </div>`;
      } else {
            ...
            btn.addEventListener("click", () => {
              ...
              switch(action) {
                ...
                case "delete":
                  ...
                  break;
                case "export":
                  const blob = new Blob([Storage.exportCapsuleJSON(Storage.loadCapsule(meta.id))], { type:"application/json" });
