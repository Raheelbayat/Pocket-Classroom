...
function createQuestionRow(q = { question: "", choices: ["", "", "", ""], correctIndex: 0, explanation: "" }) {
  const row = document.createElement("div");
  ...
  `;
  row.querySelector(".correct-select").value = q.correctIndex;
  ...
  const draft = {
    ...
    quiz: Array.from(document.getElementById("quizContainer")?.children || []).map(c => {
      ...
      return {
        ...
        correctIndex: parseInt(selects[0].value, 10),
  ...
  q.choices.forEach((choiceText, i) => {
    ...
    btn.addEventListener("click", () => {
      ...
      if (i === q.correctIndex) btn.classList.add("btn-success");
        ...
        const correctBtn = choicesContainer.querySelector(`button[data-index='${q.correctIndex}']`);
...
export function initCapsules() {
  ...
  if (path === "learn") {
    ...
    document.getElementById("showNotesBtn")?.addEventListener("click", () => { hideAllTabs(); notesContainer.style.display = "block"; });
    document.getElementById("showFlashcardsBtn")?.addEventListener("click", () => { hideAllTabs(); flashcardsContainer.style.display = "flex"; });
    document.getElementById("showQuizBtn")?.addEventListener("click", () => { hideAllTabs(); quizContainer.style.display = "block"; });
    ...
    document.addEventListener("keydown", e => {
      ...
      if (key === "[") document.getElementById("showNotesBtn")?.click();
      if (key === "]") document.getElementById("showQuizBtn")?.click();
      if (key === "/") document.getElementById("showFlashcardsBtn")?.click();
    });
